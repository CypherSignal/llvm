...
//===----------------------------------------------------------------------===//
//
// This file implements the WDC65816TargetLowering class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "WDC65816-lower"

WDC65816TargetLowering::WDC65816TargetLowering(const TargetMachine &TM,
...
SDValue WDC65816TargetLowering::LowerFormalArguments(
  ...
  case CallingConv::WDC65816_INTR:
    if (Ins.empty())
      return Chain;
...
WDC65816TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::WDC65816_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue WDC65816TargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
...
bool
WDC65816TargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
...
SDValue
WDC65816TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool isVarArg,
                                  const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  if (CallConv == CallingConv::WDC65816_INTR && !Outs.empty())
...
SDValue WDC65816TargetLowering::LowerCCCCallTo(
...
SDValue WDC65816TargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(WDC65816ISD::RRCL, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? WDC65816ISD::RLA : WDC65816ISD::RRA),
                         dl, VT, Victim);
...
SDValue WDC65816TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(WDC65816ISD::Wrapper, SDLoc(Op), PtrVT, Result);
}
...
SDValue WDC65816TargetLowering::LowerExternalSymbol(SDValue Op,
  ...
  return DAG.getNode(WDC65816ISD::Wrapper, dl, PtrVT, Result);
}
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  WDC65816CC::CondCodes TCC = WDC65816CC::COND_INVALID;
  switch (CC) {
  default: llvm_unreachable("Invalid integer condition!");
  case ISD::SETEQ:
    TCC = WDC65816CC::COND_E;     // aka COND_Z
    ...
    break;
  case ISD::SETNE:
    TCC = WDC65816CC::COND_NE;    // aka COND_NZ
    // Minor optimization: if LHS is a constant, swap operands, then the
    // constant can be folded into comparison.
    if (LHS.getOpcode() == ISD::Constant)
      std::swap(LHS, RHS);
  ...
  case ISD::SETUGE:
    ...
    if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
      ...
      TCC = WDC65816CC::COND_LO;
      break;
    }
    TCC = WDC65816CC::COND_HS;    // aka COND_C
    break;
  case ISD::SETUGT:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  case ISD::SETULT:
    ...
    if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
      LHS = RHS;
      RHS = DAG.getConstant(C->getSExtValue() + 1, dl, C->getValueType(0));
      TCC = WDC65816CC::COND_HS;
      break;
    }
    TCC = WDC65816CC::COND_LO;    // aka COND_NC
    break;
  case ISD::SETLE:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  case ISD::SETGE:
    ...
    if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
      LHS = RHS;
      RHS = DAG.getConstant(C->getSExtValue() + 1, dl, C->getValueType(0));
      TCC = WDC65816CC::COND_L;
      break;
    }
    TCC = WDC65816CC::COND_GE;
    break;
  case ISD::SETGT:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  ...
  return DAG.getNode(WDC65816ISD::CMP, dl, MVT::Glue, LHS, RHS);
}


SDValue WDC65816TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(WDC65816ISD::BR_CC, dl, Op.getValueType(),
...
SDValue WDC65816TargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue WDC65816TargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue WDC65816TargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue WDC65816TargetLowering::LowerJumpTable(SDValue Op,
    ...
    return DAG.getNode(WDC65816ISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool WDC65816TargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *WDC65816TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((WDC65816ISD::NodeType)Opcode) {
  case WDC65816ISD::FIRST_NUMBER:       break;
  case WDC65816ISD::RET_FLAG:           return "WDC65816ISD::RET_FLAG";
  case WDC65816ISD::RETI_FLAG:          return "WDC65816ISD::RETI_FLAG";
  case WDC65816ISD::RRA:                return "WDC65816ISD::RRA";
  case WDC65816ISD::RLA:                return "WDC65816ISD::RLA";
  case WDC65816ISD::RRC:                return "WDC65816ISD::RRC";
  case WDC65816ISD::RRCL:               return "WDC65816ISD::RRCL";
  case WDC65816ISD::CALL:               return "WDC65816ISD::CALL";
  case WDC65816ISD::Wrapper:            return "WDC65816ISD::Wrapper";
  case WDC65816ISD::BR_CC:              return "WDC65816ISD::BR_CC";
  case WDC65816ISD::CMP:                return "WDC65816ISD::CMP";
  case WDC65816ISD::SETCC:              return "WDC65816ISD::SETCC";
  case WDC65816ISD::SELECT_CC:          return "WDC65816ISD::SELECT_CC";
  case WDC65816ISD::DADD:               return "WDC65816ISD::DADD";
  }
  return nullptr;
}

bool WDC65816TargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool WDC65816TargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool WDC65816TargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // WDC65816 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool WDC65816TargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // WDC65816 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && VT1 == MVT::i8 && VT2 == MVT::i16;
}

bool WDC65816TargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
